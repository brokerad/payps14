...
#   <label class="string required" for="publisher_first_name">First Name <abbr title="required">*</abbr></label>
#   <input class="string required" id="publisher_first_name" maxlength="255" name="publisher[first_name]" size="50" type="text" value="Pablo">
# </div>
# <div class="clearfix">
...
SimpleForm.setup do |config|
  ...
  config.wrappers :checkboxes, :class => "clearfix", :error_class => :error do |b|
    b.use :placeholder
    b.use :label
    b.use :tag => 'div', :class => 'input' do |w1|
      w1.use :tag => 'ul', :class => 'inputs-list check-boxes' do |input|
        input.use :input
      end
      w1.use :hint,  :tag => :span, :class => :'help-block'
      w1.use :error, :tag => :span, :class => :'help-block'
    end
  end
